...
# 입력 데이터를 저장할 placeholder, 가중치a, 바이어스 b, 배추 가격을 예측하는 수식 y
X = tf.placeholder(dtype=tf.float32, shape=[None, 4]) # 변화 요인을 기억할 placeholder
a = tf.Variable(tf.random_uniform([4, 1]), dtype=tf.float32) # 기울기, 내적을 하기 위해 4행 1열의 난수를 발생시킨다. 
b = tf.Variable(tf.random_uniform([1]), dtype=tf.float32) # y절편
y = tf.matmul(X, a) + b # 예측값

# 텐서플로우 세션을 선언하고 변수 초기
sess = tf.Session()
...
# flask의 기본 포트인 5000번 포트의 기본 경로('/')에 사용자가 GET, POST 방식으로 요청했을 때 render_template() 함수를 이용해서
# 특정 페이지(index.html)를 브라우저에 표시할 수 있도록 한다.
...
def index():
    ...
    # POST 방식으로 요청하면 사용자가 입력한 데이터를 받는다. => submit 버튼을 클릭했을 때
        ...
        # form에 입력한 데이터를 받는다.
        avg_temp = float(request.form['avg_temp']) # 평균 기온을 받는다.
        min_temp = float(request.form['min_temp']) # 최저 기온을 받는다.
        max_temp = float(request.form['max_temp']) # 최고 기온을 받는다.
        rain_fall = float(request.form['rain_fall']) # 강수량을 받는다.
